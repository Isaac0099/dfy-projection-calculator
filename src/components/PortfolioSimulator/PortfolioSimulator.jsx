...
const PortfolioSimulator = () => {
  ...
  const handleCalculate = (data) => {
    const results = runSimulation(
      ...
      data.legacyYears,
      data.yearsBetweenRefinances,
      data.percentAppreciationToWithdraw / 100,
      data.retirementIncomeStrategy,
      data.growthStrategy
    );
    setSimulationData({
      homes: data.homes,
  ...
  return (
    <div className="p-4">
      {!showResults ? (
        <HomeListBuilder onCalculate={handleCalculate} initialData={simulationData} />
      ) : (
        <SimulationResults
