...
const ComparisonChart = ({ projectionYears, equityData, initialHomes, results }) => {
  ...
  const generateData = () => {
    const data = [];
    const stockMarketReturn = 1.1; // 10% annual return
    ...
    <div className="h-96">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={generateData()} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          ...
          <Line type="monotone" dataKey="Stock Market" stroke="#84cc16" strokeWidth={1} dot={false} />
          <Line type="monotone" dataKey="Mixed" stroke="#0d9488" strokeWidth={1} dot={false} />
          <Line type="monotone" dataKey="Bonds" stroke="#1d4ed8" strokeWidth={1} dot={false} />
          <Line type="monotone" dataKey="Savings Account" stroke="#64748b" strokeWidth={1} dot={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
